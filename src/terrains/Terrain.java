...
public class Terrain {
	...
	public Terrain(int gridX, int gridY, Loader loader,
	...
	public boolean isCollidedWithTerrain(float x, float z) {
		if(z >= this.getZ()
				&& z <= this.getZ() + getTileSize()
				&&	x >= this.getX() &&
				x <= this.getX() + this.getTileSize())
			return true;
		return false;
	}
	
	public float getHeightOfTerrain(float worldX, float worldZ) {
		//World coordinate to pos relative to terrain
		float terrainX = worldX - this.x;
		float terrainZ = worldZ - this.z;
		//for 4 vertexes only 3 squares
		float gridSquareSize = SIZE / (float)(heights.length - 1);
		int gridX = (int)Math.floor(terrainX / gridSquareSize);
		int gridZ = (int)Math.floor(terrainZ / gridSquareSize);
		//return heights[gridX][gridZ];
		if(gridX >= (heights.length - 1) || gridZ >= heights.length -1 || 
				gridX < 0 || gridZ < 0) return 0;
		//coord of the player in the square
		
		float xCoord = (terrainX %  gridSquareSize) / gridSquareSize;
		float zCoord = (terrainZ %  gridSquareSize) / gridSquareSize;
		//taking triangle using x = 1 - z . (x < 1 - z or x > 1 - z)
		float answer;
		if(xCoord <= (1-zCoord)) {
